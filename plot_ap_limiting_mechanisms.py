...
def save(i = [1]):
    plt.gca().set_xticks(range(i[0]))
    plt.gca().set_xticklabels(["All", "Two\nResources", "One\nResource",
                           "No Predation"
                           ,"No Predation\nTwo\nResources",
                           "No Predation\nOne\nResource"][:i[0]])
    fig.savefig("PP_limiting_mechanisms_{}.png".format(i[0]))
    i[0] += 1
    
plt.plot(0, richness.loc["NPLZ", "phyto"], 'bo')
save()

plt.plot(1+dx, richness.loc[["NPZ", "NLZ", "PLZ"], "phyto"], "bo")
save()

plt.plot(2+dx, richness.loc[["NZ", "PZ", "LZ"], "phyto"], "bo")
save()

plt.plot(3, richness.loc["NPL", "phyto"], "bo")
save()

plt.plot(4 + dx, richness.loc[["NP", "NL", "PL"], "phyto"], "bo")
save()

plt.plot(5+dx, richness.loc[["N", "P", "L"], "phyto"], "bo")
save()